...
public class CachedPlacesProvider extends ContentProvider {
    ...
    static {
        ...
        CONTACT_URI = Uri.withAppendedPath(AUTHORITY_URI, "contact");
    ...
    public Cursor query(Uri uri, String[] projection,
        ...
        Cursor cursor = null;
        if (sURIMatcher.match(uri) == URI_CONTACT_WITH_ID) {
            String normalizedNumber = getNumberFromUri(uri);
            if (normalizedNumber != null)
            {
                mDatabaseHelper.deleteOldNumbers();
                ...
                cursor = mDatabaseHelper.getWritableDatabase().query(
                        ...
                        CachedPlacesDatabaseHelper.NORMALIZED_NUMBER + "=?", args,
                        null, null, null);
            }
        }
        return cursor;
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (match != URI_CONTACT && match != URI_CONTACT_WITH_ID) {
        ...
        if (match == URI_CONTACT) {
            normalizedNumber = getNumberFromValues(values);
        } else {
            normalizedNumber = getNumberFromUri(uri);
        }

        values.put(CachedPlacesDatabaseHelper.NORMALIZED_NUMBER, normalizedNumber);
        ...
        long row = mDatabaseHelper.getWritableDatabase().insertWithOnConflict(
                CachedPlacesDatabaseHelper.TABLE, null, values, SQLiteDatabase.CONFLICT_REPLACE);

        if (row < 0) {
            Log.e(TAG, "Could not cache place with number " + normalizedNumber);
        }
    ...
    private String getNumberFromValues(ContentValues values) {
        String phoneNumber = values.getAsString(Place.PHONE_NUMBER);
    ...
    private ParcelFileDescriptor openImageForWrite(String phoneNumber) {
        ...
        try {
            ...
            Log.w(TAG, "Failed to create new file for cached image: " + file);
        }
        return fileDescriptor;
    }

    private void setHasImage(String phoneNumber, boolean hasImage) {
