...
public class UsWhitePages implements IReverseLookupApi {

    private final static String TAG = UsWhitePages.class.getSimpleName();
    ...
    public Place getNamedPlaceByNumber(PhoneNumber phoneNumber) {
        ...
        try {
            ...
            if (matcher.find()) {
                ...
                matcher.find();
                place.setName(matcher.group(1).trim());

                regex = Pattern.compile(">\\s*Business\\s*<", Pattern.DOTALL);
                matcher = regex.matcher(associated);
                place.setBusiness(matcher.find());
                ...
                if (matcher.find()) {
                    place.setStreet(matcher.group(1).trim());
                }
                ...
                matcher = regex.matcher(html);
                if (matcher.find()) {
                    place.setCity(matcher.group(1).trim());
                    place.setPostalCode(matcher.group(2).trim());
                }
            } else {
