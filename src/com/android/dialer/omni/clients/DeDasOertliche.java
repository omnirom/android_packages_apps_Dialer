...
public class DeDasOertliche implements IReverseLookupApi {

    private final static String TAG = ChSearch.class.getSimpleName();
    private final static String QUERY_URL = "http://www.dasoertliche.de/Controller?form_name=search_inv&ph=";

    // testNumber = "+49874196420";
    ...
    public Place getNamedPlaceByNumber(PhoneNumber phoneNumber) {
        ...
        try {
            ...
            if (!TextUtils.isEmpty(htmlDocumentHandler.name)) {
                place = new Place();
                place.setName(htmlDocumentHandler.name);
                place.setPhoneNumber(normalizedNumber);
                place.setStreet(htmlDocumentHandler.address);
                place.setPostalCode(htmlDocumentHandler.postalCode);
                place.setCity(htmlDocumentHandler.city);
                place.setEmail(htmlDocumentHandler.email);
                place.setWebsite(htmlDocumentHandler.website);
                place.setPhoneType(Phone.TYPE_WORK);
            ...
            if (length > 0) {
                if (elementIs("div")
                    ...
                    if (name == null
                        ...
                        address = getValue(ch, start, length).split(",")[0].trim();
