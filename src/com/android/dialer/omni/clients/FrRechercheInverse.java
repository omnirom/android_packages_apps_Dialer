...
public class FrRechercheInverse implements IReverseLookupApi {

    private final static String TAG = FrRechercheInverse.class.getSimpleName();
    ...
    public Place getNamedPlaceByNumber(PhoneNumber phoneNumber) {
        ...
        try {
            JSONObject obj = PlaceUtil.getJsonRequest(QUERY_URL + encodedNumber);
            ...
            if (obj.getInt("nb") > 0) {
                ...
                if (inverse.getString("NNAT").replaceAll(" ", "").equals(nationalNumber)) {
                    ...
                    if (inverse.has("ADRESSE")) {
                        place.setStreet(inverse.getString("ADRESSE"));
                    }
                    if (TextUtils.isEmpty(place.getStreet())
                            && inverse.has("NOMV")) {
                        place.setStreet(inverse.getString("NOMV"));
                    }
                    if (inverse.has("CP")) {
                        place.setPostalCode(inverse.getString("CP"));
                    }
                    ...
                    if (TextUtils.isEmpty(place.getCity())
                            && inverse.has("STRDEP")) {
                        place.setCity(inverse.getString("STRDEP"));
                    }
                } else {
